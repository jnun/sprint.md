...
for folder in "${SPRINTBIAS_STAGES[@]}"; do
...
if git status --porcelain docs/sprintbias/DOC_STATE.md 2>/dev/null | grep -q .; then
    git add docs/sprintbias/DOC_STATE.md 2>/dev/null || true
