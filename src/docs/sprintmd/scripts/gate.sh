...
if [ "$FOLDER" != "next" ]; then
  ...
  if [ "$AI_MODE" != "emit" ]; then
    command -v "$SPRINTMD_CLI" &>/dev/null || {
      echo "Error: AI CLI '$SPRINTMD_CLI' not found in PATH. Edit docs/sprintmd/config (CLI) or install the tool. Claude Code: https://docs.anthropic.com/en/docs/claude-code/overview" >&2
      exit 1
    }
  fi
  ...
  if [ "$AI_MODE" = "emit" ]; then
    ...
    _file_list=$(printf '%s\n' "${files[@]}")
    sprintmd_run -p "You are vetting task-file quality in $FOLDER/ for the developer.
...
_chat_queue() {
  ...
  for d in $(printf '%s' "$all_deps" | tr ' ' '\n' | grep -E '^[0-9]+$' | sort -un); do
    ...
    _res="$(sprintmd_find_task "$d")" || continue
    dfile="${_res%%$'\t'*}"
    [ "$(sprintmd_review_verdict "$dfile")" = "READY" ] && continue
    upstream="$upstream $d"
  done
...
for i in $(seq 0 $((COUNT - 1))); do
  ...
  sprintmd_gate_review "$NEXT_DIR/$TASK_NAME"
  ...
  case "$SPRINTMD_GATE_VERDICT" in
    ...
    READY)
      ...
      ;;
    NOSTAMP)
      ERROR_TASKS+=("$TASK_NAME → no verdict stamp, log: $SPRINTMD_GATE_LOG")
      echo ""
      echo "✗ No verdict found in $TASK_NAME — leaving in $NEXT_DIR"
      echo "  Log: $SPRINTMD_GATE_LOG"
      ;;
    FAILED)
      ERROR_TASKS+=("$TASK_NAME → ${SPRINTMD_GATE_ERROR:-review failed}, log: $SPRINTMD_GATE_LOG")
      echo ""
      echo "✗ Review failed for $TASK_NAME — left in $NEXT_DIR"
      [ -n "$SPRINTMD_GATE_ERROR" ] && echo "  Cause: $SPRINTMD_GATE_ERROR"
      echo "  Log: $SPRINTMD_GATE_LOG"
      ;;
  esac
