#!/usr/bin/env bash
set -euo pipefail
# ai-context.sh — Generate AI context summary. See: ./sprint.sh help ai-context
...
if [ -f "$DOCS_DIR/sprintmd/DOC_STATE.md" ]; then
    cat "$DOCS_DIR/sprintmd/DOC_STATE.md"
else
...
echo "## Epics (Relational Groupings)"
# An epic is a named index over tasks, NOT a lifecycle stage and NOT a task.
# List each epic and resolve its "- #ID" members to their current folder so
# the grouping's progress is visible without ever counting the epic as a task.
if [ -d "$DOCS_DIR/epics" ]; then
    _epic_any=0
    for _sf in "$DOCS_DIR"/epics/*.md; do
        [ -f "$_sf" ] || continue
...
if [ "$blocked_count" -gt 0 ]; then
    ...
    echo "Run './sprint.sh talk <task-id>' on a blocked task to work out why it's stuck."
