#!/usr/bin/env bash
# talk-sprint.sh — Walk the whole sprint through, not one task. A deterministic
...
# those findings one at a time — same one-detail voice as single-task `talk`.
#
# Reached via `./sprint.sh talk` with NO task id (talk.sh routes here). This is a
# STRUCTURAL health pass ("is this sprint internally consistent and unblocked?"),
# deliberately distinct from `review-sprint` (a planning critique). See:
#   ./sprint.sh help talk
#
...
# rule fiveday_unmet_deps encodes. Reciprocity/cycle checks only look inward at
...
# way talk.sh and the audit scripts do. Always exit 0 so set -e never trips on a
...
# Iterate FIVEDAY_STAGES rather than hardcoding folder names (lib.sh:251).
build_index() {
...
#   (a) the '### Questions for the developer' subsection (define/talk convention);
...
for id in $(board_ids); do
  ...
  verdict="$(fiveday_review_verdict "$file")"
  ...
  for p in $parent; do
    ...
    add_finding 4 HYGIENE "$id" "$file" \
      ...
      "point Parent at the real epic, or set it to none"
  done
  ...
  while IFS= read -r q; do
    ...
    if [ "$stage" = "next" ] && [ "$verdict" = "READY" ]; then
      add_finding 2 INTEGRITY "$id" "$file" \
        ...
        "answer/decide it and write the resolution back into the file, or 'talk $id' to define it"
    else
  ...
  if [ "$stage" = "next" ]; then
    ...
    if [ "$verdict" != "READY" ]; then
      add_finding 4 HYGIENE "$id" "$file" \
        ...
        "run 'talk $id' to finish defining it, or 'define' to vet it"
    fi
    # Dependency-stage violations — grade each unmet dep by where it sits.
    for u in $(fiveday_unmet_deps "$file"); do
      ustage="$(stage_of "$u")"
      case "$ustage" in
        blocked)
          add_finding 1 BLOCKER "$id" "$file" \
            "depends on $u, which is BLOCKED — this task cannot actually run yet" \
            "PATH A: define $u so it can leave blocked/ (hands off to 'talk $u'); or PATH B: demote this task to backlog/ so the sprint holds no work blocked on $u" ;;
        backlog)
          add_finding 3 ORDERING "$id" "$file" \
...
# ── Workable frontier ────────────────────────────────────────────────
# Mirror tasks.sh's real gate exactly (readiness + unmet-dep check) so the
...
for id in $(printf '%s\n' "$INDEX" | awk -F'\t' '$2=="next"{print $1}' | sort -n); do
  file="$(file_of "$id")"
  unmet="$(fiveday_unmet_deps "$file")"
  if [ "$(fiveday_review_verdict "$file")" != "READY" ]; then
...
if [ "$FINDING_COUNT" -eq 0 ]; then
  ...
  echo "Start the sprint whenever you're ready:  ./sprint.sh tasks"
  exit 0
fi
...
# with the talk-next folder walk (task 226). It lives in lib.sh so both entry
# points walk the edge identically — do not inline a second copy here.
NEXT_BLOCKED_RESOLUTION="$(fiveday_next_blocked_resolution)"

APPEND_PROMPT="You are a senior engineer running a stand-up over a queued sprint with the colleague who owns it. This is a STRUCTURAL HEALTH pass — is the sprint internally consistent and unblocked? — NOT a planning critique of whether it is the right sprint (that is './sprint.sh review-sprint'; do not duplicate it). A deterministic shell preflight has already found every issue below; your job is to WALK them with the user and fix each, one at a time, in the same one-detail-at-a-time voice as single-task talk.

SPRINT STATE
$NEXT_COUNT tasks queued in $NEXT_DIR/.
$FRONTIER_BLOCK

FINDINGS (already computed, most-blocking first — do not re-scan the board):
$FINDINGS_RENDERED

HOW TO OPEN
Give a summary of at most three lines: the queued count, the runnable frontier ids, and the number of findings. Then start walking findings — do not dump them all at once.

HOW TO WALK — one finding at a time, in the order listed (most-blocking first)
For EACH finding:
1. VERIFY it against the actual task file before saying anything — open the file named in the finding and confirm the issue is real. The preflight is deterministic but conservative; if a finding is already resolved in the file (e.g. a question marked RESOLVED), say so in one line and move to the next. Never act on a finding you could not confirm.
2. STATE it plainly in one or two sentences — what is wrong and why it matters for running the sprint.
3. RECOMMEND a specific fix (the finding carries a suggested one) and OFFER TO ACT. Acting means one of, and only these, staying strictly inside the task pipeline:
   - fix a metadata edge (correct or reciprocate a Depends on / Blocks / Parent line);
   - resolve a mis-parked blocked task: a blocked/ file with no '**Status: BLOCKED**' stamp and no '## Questions' is almost always a mistake — the DEFAULT is to move it back to backlog/ to be reconsidered or archived later ('git mv $BLOCKED_DIR/<file> docs/tasks/backlog/<file>'). When in doubt, backlog is the safe home. Only record a BLOCKED reason (a '## Questions' section whose first line is exactly '**Status: BLOCKED**') or promote it to next/ when there is a clear, stated reason to;
   - stamp or correct a '**Status:**' marker;
   - resolve a next→blocked BLOCKER finding (a next/ task depends on a task still in blocked/) via the two-path choice — follow 'RESOLVING A next→blocked BLOCKER' below exactly; do NOT paper over it by dropping the Depends on line;
   - for anything needing real definition work rather than a one-line fix, CHAIN OUT: tell the user to run './sprint.sh talk <id>' in a fresh window — that command owns the deep refine/close-the-loop logic; do not try to redefine a task inline here.
4. On an OPEN DECISION, lay out the realistic choices in a sentence each, say which you would pick and why, and offer to apply the sensible default rather than making the user adjudicate. If they defer, choose it and record a one-line rationale.
5. MOVE ON — note what is settled, then the next finding.

OUTSTANDING QUESTIONS — the write-back loop (do this, do not merely discuss)
When a finding is an outstanding question, surface it verbatim as: Task <id> has an outstanding question: \"<question text>\". Give the user a chance to ANSWER, DECIDE, or give FEEDBACK. Turn their response into a conversational update WRITTEN BACK INTO THE TASK FILE:
   - replace the open question with a resolved decision (record it under a '### Resolved decisions (<date>, by the developer)' list, the shape task 222 uses), and
   - once no open question remains, flip the task's '**Status:**' marker to READY.
If the question needs real definition work rather than a one-line answer, chain into './sprint.sh talk <id>' instead of answering inline.

$NEXT_BLOCKED_RESOLUTION

RULES
- One finding at a time; wait for the user between findings. Keep the conversation moving — do not parrot their words back.
- Executive-summary altitude: what and why, not how. No code snippets.
- WRITES stay within the task pipeline — you may edit task files under docs/tasks/ and move them between stage folders (prefer 'git mv', fall back to 'mv' if the file is uncommitted). Read anything to verify, write nothing else.
- Prefer 'git mv' for moves so history follows the file; if it fails because the file is uncommitted, use plain 'mv'.
- When every finding is walked, give a one-paragraph recap: what was fixed, what was chained out to 'talk <id>', and the final runnable frontier. Then stop."

# ── Emit vs exec: same interactivity contract as talk.sh ─────────────
...
# fiveday_interactive_ok that routes the run decides the warning, so they agree.
...
fiveday_run_interactive \
  ...
  --permissions "auto" \
  --name "talk-sprint" \
  "Walk this sprint through: open with the ≤3-line summary, then take the findings one at a time, most-blocking first — verify each against its file, then fix it or chain into 'talk <id>'."
