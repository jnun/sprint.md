...
#   bash docs/sprintmd/scripts/promote-to-sprint.sh <task-file>
#
...
if [ -z "$FILE" ] || [ "$FILE" = "-h" ] || [ "$FILE" = "--help" ]; then
  echo "Usage: bash docs/sprintmd/scripts/promote-to-sprint.sh <task-file>"
  echo "  Runs the shared workability gate; READY → next/, BLOCKED → blocked/,"
  echo "  COMPLETE → review/. Never raw-moves into next/."
  exit 1
fi

if ! sprintmd_promote_to_sprint "$FILE" promote; then
  sprintmd_promote_summary "$(basename "$FILE")"
  exit 1
fi
sprintmd_promote_summary "$(basename "$FILE")"
