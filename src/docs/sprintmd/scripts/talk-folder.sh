...
# talk-folder.sh — Express one-at-a-time sweep of a single stage folder
# (blocked/, next/, or backlog/). Reached via `./sprint.sh talk <stage>` —
# talk.sh's dispatcher validates the stage name and routes here. This is the
# THIRD case of talk's grammar, completing `talk [target]`:
#   talk            → walk the whole sprint's structural health (talk-sprint.sh)
#   talk <id>       → talk one task through (talk.sh)
#   talk <folder>   → this: a fast verdict-first sweep of one stage folder
#
...
# escalating a task to "define it" hands it to talk.sh, whose full conversation
# runs on the strongest TALK model. Fast cheap sort by default, full depth only
# where you ask for it.
#
# Dependency resolution is INTRINSIC to talk and runs on every file the sweep
# analyzes, exactly as it does for `talk <id>` and the no-arg walk: whenever a
# swept task's `Depends on` points into blocked/, the sweep lifts and defines
# that dependency (via the shared fiveday_next_blocked_resolution helper and
# talk's own fresh-context chain) so the dependent task can actually be worked.
# The folder argument only selects WHICH files are swept.  See: help talk
...
case "$STAGE" in
  ...
  *)
    echo "Usage: talk <folder>   (folder must be one of: blocked, next, backlog)"
    exit 1 ;;
esac
...
# "define it" path shells to talk.sh, which picks the strong TALK model itself.
...
# Written once in lib.sh so the no-arg sprint walk, `talk <id>`, and this folder
...
blocked_deps() {
  ...
  for u in $(fiveday_unmet_deps "$file"); do
    if fiveday_find_task "$u" docs/tasks/blocked >/dev/null 2>&1; then
      out="$out $u"
    fi
  done
  printf '%s' "${out# }"
...
if [ "$AI_MODE" = "emit" ]; then
  _file_list=$(printf '%s\n' "${all_files[@]}")
  fiveday_run -p "You are sweeping the $STAGE/ task folder with the developer, one task at a time — a fast verdict-first sort, NOT a full conversation on every file. Rip through the queue; go deep only where asked.
...
3. Offer the developer the choice:
   [w] work it   — promote (blocked|backlog → next/) or start (next/ → doing/), via git mv
   [d] define it — go DEEP: run './sprint.sh talk <id>' so the full talk conversation (strongest model) refines the task and closes the loop. This is the only step that escalates past the fast verdict.
   [k] kill it   — delete after confirming (git rm)
   [s] skip      — leave it where it is
   [q] quit      — stop the sweep
4. Act on the choice within the task pipeline (prefer 'git mv' / 'git rm'; fall back to plain 'mv' / 'rm' when the file is uncommitted), then continue to the next task.

DEPENDENCY RESOLUTION — intrinsic to talk, applied to EVERY task you sweep (not only on [d]):
When a swept task's '**Depends on**:' names a task that currently sits in blocked/, that dependent task can NEVER be worked until the dependency is defined and leaves blocked/. Do not just note it — resolve it, following the shared rule below. The folder you are sweeping only chose WHICH files to open; what you do to each is the same as 'talk <id>'.
...
- Do not output anything else"
  ...
  # (talk.sh's chain lifts the most-upstream one first).
  ...
  case "$choice" in
    ...
    d|D)
      ...
      if [ -n "$task_id" ]; then
        echo -e "  ${BLUE}-> Going deep: launching talk on $task_id...${NC}"
        # talk.sh runs the full conversation on the strong TALK model AND, being
        # talk, resolves this task's blocked dependencies via its own fresh-
        # context chain — the intrinsic dep resolution, not reimplemented here.
        bash "$(dirname "${BASH_SOURCE[0]}")/talk.sh" "$task_id" </dev/tty || true
        echo ""
