...
for arg in "$@"; do
  ...
  case "$arg" in
    --drift)      SPRINTMD_SKIP_DRIFT_CHECK=0 ;;
    --audit)      RUN_AUDIT=1 ;;
...
if [ "${_ASSIST:-0}" -eq 1 ]; then
  ...
  _NO_LIMITS=0; SPRINTMD_SKIP_DRIFT_CHECK=1
  for arg in "$@"; do
    case "$arg" in
      --drift)      SPRINTMD_SKIP_DRIFT_CHECK=0 ;;
      --audit)      RUN_AUDIT=1 ;;
...
if [ "$_NO_LIMITS" -eq 1 ]; then
  SPRINTMD_BUDGET_WORK=""
fi
unset _NO_LIMITS

# ── Preflight ───────────────────────────────────────────────────────
# run_with_timeout comes from lib.sh. In emit mode no CLI binary is needed;
# in exec mode sprintmd_ai_mode already verified the binary exists.
DRIFT_TIMEOUT=120
...
if [ "$FORCE" -ne 1 ]; then
  ...
  for _f in "${TASK_FILES[@]}"; do
    if [ "$(sprintmd_review_verdict "$_f")" != "READY" ]; then
      _skipped+=("$_f")
    else
      _defined+=("$_f")
    fi
...
# check is filesystem-based (sprintmd_unmet_deps scans backlog/next/doing/blocked),
...
- Before your verdict, list any fixes you made as bullet points (for FIXED)"
    ...
    DRIFT_VERDICT=$(run_with_timeout "$DRIFT_TIMEOUT" sprintmd_run -p "$DRIFT_PROMPT" \
      "${_drift_model_args[@]}" \
