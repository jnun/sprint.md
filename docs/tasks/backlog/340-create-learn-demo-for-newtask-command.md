...
**Perspective check.**
- *Chief Platform Architect:* `newtask` is the atom of the whole system, so teaching it matters — but it has near-zero integrity surface and is undramatic in isolation. A whole demo for "create one task" is thin theater.
- *Chief Experience Officer:* Fast, frictionless capture is core to the promise, yet watching a single `newtask` run is not a *situation* — there's no tension, no payoff. It only sings as one beat inside a larger session.

**Tension and resolution.** Both agree the standalone demo is too small to carry a story. Resolution: `newtask` is a **beat inside `session.py` (S0)** — "one problem, one session" already has to create a task to exist. Cut the standalone; make sure S0 names the command clearly as it happens.

**Sharper rewrite (only if kept):** *Problem:* new users hesitate that capture is heavyweight. *Success:* S0 visibly runs `newtask <description>` and lands it in `backlog/`, no new file, no separate mapping.
