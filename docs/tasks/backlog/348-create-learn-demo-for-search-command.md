# Task 348: Create learn demo for search command
...
**Perspective check.**
- *Chief Platform Architect:* `search` is a read-only keyword lookup — zero integrity surface, nothing to sandbox-prove, and nothing that can go wrong. There is no system-health lesson to teach here.
- *Chief Experience Officer:* Nobody onboards by *watching* a search. It's a utility users discover the instant they need it; a curated demo slot spent on it is a slot stolen from a story that actually converts a newcomer.

**Tension and resolution.** Unanimous. Resolution: **cut.** A one-line mention in `--help`/DOCUMENTATION.md fully covers `search`; a demo adds catalog noise against the README's explicit "grow without rot" mandate.
