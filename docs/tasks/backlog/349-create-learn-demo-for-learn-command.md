# Task 349: Create learn demo for learn command
...
**Perspective check.**
- *Chief Platform Architect:* A demo *of the demo player* is self-referential and near-useless — and it's the one command that can't cleanly sandbox itself (playing `learn` inside `learn`). It adds a file whose whole content is "here is how to watch files like me." Pure rot.
- *Chief Experience Officer:* Users discover `learn` by *running* it — `./sprint.sh learn` already lists the catalog in plain language. Explaining the catalog with an item *inside* the catalog is circular and would confuse the very newcomer it targets.

**Tension and resolution.** No tension — this is the plan's clearest cut. Resolution: **remove #349 entirely.** The catalog listing (`learn` with no arg) and `learn --help` are the correct, non-recursive way to teach `learn`.
