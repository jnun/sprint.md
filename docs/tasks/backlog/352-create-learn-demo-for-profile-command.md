# Task 352: Create learn demo for profile command
...
**Perspective check.**
- *Chief Platform Architect:* `profile` shapes provider/model behavior — config with real downstream effects on every AI call. Teaching it matters, but it's an *interactive* setup command; the demos' trust contract forbids writes, so a demo would have to fake a form it can't actually fill. Faking config risks teaching a false mental model.
- *Chief Experience Officer:* A demo of a settings form is dull, and setup friction is better killed by GETSTARTED/`--help` than by watching someone else type answers. The onboarding win here is a good first-run *wizard*, not a spectator demo.

**Tension and resolution.** Both land on cut-as-demo. Resolution: **cut;** serve `profile` through GETSTARTED and `profile --help` / `profile show`, where it's actionable rather than theatrical.
