...
**Tension and resolution.** No conflict — both rank this low. Resolution: **defer.** Don't build a `split` demo until the core capture→convert→plan→automate curriculum is complete; revisit only if user feedback shows people hit oversized tasks and miss the tool.

**Sharper rewrite (if ever built):** *Problem:* users don't know a too-big task can be broken up in one command. *Success:* a short demo shows one large task fanning into subtasks via `split <path>`.
