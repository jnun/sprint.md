# Task 351: Create learn demo for context command
...
**Perspective check.**
- *Chief Platform Architect:* `context` generates an AI context summary — plumbing that another agent consumes, not a user-facing flow. There's nothing to *watch* and no integrity story; a demo would narrate an internal artifact.
- *Chief Experience Officer:* This is invisible-to-the-user machinery. Turning it into theater risks implying it's a step users must take, adding perceived process where the value is that it's automatic.

**Tension and resolution.** Both agree it's not demo material. Resolution: **cut.** Document `context` in the manual; keep it out of the learning catalog.
