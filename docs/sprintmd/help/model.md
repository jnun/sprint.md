...
pure config over docs/sprintmd/config (keep family).
...
What it does:
  show  — Prints the active CLI, provider tier, and mode, then the effective
          ...
          plan_think, drift) with WHERE that model comes from:
            env <VAR>          a this-shell SPRINTMD_MODEL_<ROLE> override
            config MODEL_<ROLE> a per-role pin in docs/sprintmd/config
  ...
  set   — Writes MODEL_DEFAULT (KEY = default) or MODEL_<ROLE> into
          docs/sprintmd/config, leaving every other key untouched. An empty
...
Model layering (highest precedence first):
  ...
  2. --model <id> flag            per-run lever (exports SPRINTMD_MODEL_DEFAULT
...
SPRINTMD_MODEL_<ROLE> already exported in your shell.
