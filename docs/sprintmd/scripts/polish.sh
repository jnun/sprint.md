...
#       Never edits product code. Protocol: docs/sprintmd/ai/audit-excellence.md
#
#   polish --code <file|task.md> [file...] [-- max-passes]
#       Code-diff audit: fixer/verifier loop that may fix issues inline.
#
# Argument shape selects the mode: a bare number is a sweep limit; a bare
# path is the deep judge; the same path with --code is the code audit.
# See: ./sprint.sh help polish
...
_NO_LIMITS=0
FORCE=0
MAX_PASSES="${SPRINTMD_AUDIT_MAX_PASSES:-3}"
POSITIONAL=()
...
Usage:
  ...
  ./sprint.sh polish <task.md|file> [file...]
      Deep-judge one finished piece; file enhancements to backlog/.

  ./sprint.sh polish --code <task.md|file> [file...] [-- max-passes]
...
for p in "${POSITIONAL[@]+"${POSITIONAL[@]}"}"; do
  if [ -f "$p" ]; then
    ...
    # Bare number: sweep limit, or max-passes when --code already has a target.
...
if [ "$MODE" = "code" ]; then
  ...
  if [ "$AI_MODE" != "emit" ] && ! command -v "$SPRINTMD_CLI" &>/dev/null; then
    echo "✗ AI CLI '$SPRINTMD_CLI' not found in PATH" >&2
  ...
  sprintmd_change_manifest "$TASK_FILE" ${EXPLICIT_FILES[@]+"${EXPLICIT_FILES[@]}"}
  CHANGED_FILES="$SPRINTMD_CHANGED_FILES"
  CONTEXT_SOURCE="$SPRINTMD_CONTEXT_SOURCE"
  ...
  if [ "$AI_MODE" = "emit" ]; then
    sprintmd_run -p "You are auditing the code changes below for the developer.
...
---"
  ...
  _budget_args=()
  [ -n "${SPRINTMD_BUDGET_AUDIT:-}" ] && _budget_args=(--budget "$SPRINTMD_BUDGET_AUDIT")
...
FAIL: couldn't fix all · BLOCKED: needs a human."
    ...
    STEP_VERDICT=$(printf '%s' "$OUTPUT" | sprintmd_parse_verdict 'PASS|FIXED|FAIL|BLOCKED')
    ...
    PREV_SUMMARY=$(sprintmd_extract_summary "$LOG_FILE")
  ...
  case "$VERDICT" in
    ...
    UNCLEAR)
      ...
      if [ -n "${LOG_FILE:-}" ] && [ ! -s "$LOG_FILE" ]; then
        echo "  Last log is empty — the AI CLI likely failed to start (check '$SPRINTMD_CLI' install/auth)"
      else
...
intended goal from the code and recent git history."
  ...
  PROFILE_LINE="$(sprintmd_profile_line)"
...
5. Output the report per the protocol. Your VERY LAST line must be the verdict
  ...
  LOG_FILE="$(sprintmd_log_path polish-judge "${TASK_NAME:-adhoc}")"
  ...
  VERDICT=$(printf '%s' "$OUTPUT" | sprintmd_parse_verdict 'EXCELLENT|FILED|BLOCKER')
  [ -z "$VERDICT" ] && VERDICT="UNCLEAR"

  SUMMARY=$(sprintmd_extract_summary "$LOG_FILE")
  ...
  case "$VERDICT" in
    ...
    *)
      ...
      if [ ! -s "$LOG_FILE" ]; then
        echo "  Log is empty — the AI CLI likely failed to start (check '$SPRINTMD_CLI' install/auth)"
      fi
      exit 1
      ;;
  esac
...
_refine_prompt() {
  ...
  sprintmd_change_manifest "$task_file"
  local changed="$SPRINTMD_CHANGED_FILES"
  local ctx="$SPRINTMD_CONTEXT_SOURCE"
  local profile_line; profile_line="$(sprintmd_profile_line)"
...
_budget_args=(); [ -n "${SPRINTMD_BUDGET_AUDIT:-}" ] && _budget_args=(--budget "$SPRINTMD_BUDGET_AUDIT")
...
for ((i=0; i<COUNT; i++)); do
  ...
  LOG_FILE="$(sprintmd_log_path polish "$TASK_NAME")"

  OUTPUT=$(sprintmd_run -p "$(_refine_prompt "$TASK_FILE" "$NEXT_ROUND")" \
    ${_model_args[@]+"${_model_args[@]}"} \
    ${_budget_args[@]+"${_budget_args[@]}"} \
    --tools "$TOOLS" \
    --permissions "$PERMISSIONS" \
  ...
  VERDICT=$(printf '%s' "$OUTPUT" | sprintmd_parse_verdict 'PASS|REOPEN|BLOCKER')
  [ -z "$VERDICT" ] && VERDICT="UNCLEAR"
  ...
  case "$VERDICT" in
    ...
    *)
      ...
      [ -s "$LOG_FILE" ] || echo "    Log is empty — the AI CLI likely failed to start (check '$SPRINTMD_CLI')"
      ;;
  esac
