...
# judge impact. See: ./sprint.sh help audit-deps
...
# stable, grouped output. Override the vendor list with FIVEDAY_DEPS_PRUNE.
PRUNE_DIRS="${FIVEDAY_DEPS_PRUNE:-node_modules vendor .git dist build out target .venv venv env .tox .next .nuxt .svelte-kit .cache coverage bower_components Pods .terraform __pycache__ .gradle}"
...
MAX_PROJECTS="${FIVEDAY_DEPS_MAX_PROJECTS:-25}"
PROCESSED=""
COUNT=0
SKIPPED=0
...
if [ "$SKIPPED" -gt 0 ]; then
  ...
  echo "  Raise it with FIVEDAY_DEPS_MAX_PROJECTS=<n> to cover them."
fi
...
# is the AI's (or a human's) source material and can be trimmed once analysed.
{
  ...
  echo ""
  echo "_Generated by \`./sprint.sh audit-deps\` on $(date +%Y-%m-%d). Delete once the sections above are filled._"
  cat "$RAW_LOG"
} >> "$TASK_FILE"
