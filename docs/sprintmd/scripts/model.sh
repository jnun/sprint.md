...
# pure config plumbing over docs/sprintmd/config. See: ./sprint.sh help model
#
...
#   1. env  SPRINTMD_MODEL_<ROLE>   (this-shell override, never written to disk)
#   2. --model <id> flag            (per-run lever a spine command exports as
#                                    SPRINTMD_MODEL_DEFAULT for one invocation)
#   3. config MODEL_<ROLE>          (per-role pin in docs/sprintmd/config)
#   4. config MODEL_DEFAULT         (global pin)
# and, when all are empty on an orchestration tier, a strong tier default
# (claude-code → opus, grok-build → grok-4.5) via sprintmd_tier_model.
...
# The known role suffixes — one per MODEL_<ROLE> key in docs/sprintmd/config.
...
role_line() {
  local sfx="$1" env_var="SPRINTMD_MODEL_$1" model source
  model="$(sprintmd_tier_model "$sfx")"
  ...
  elif [ -n "$(sprintmd_cfg "MODEL_$sfx")" ]; then
    source="config MODEL_$sfx"
  elif [ -n "$(sprintmd_cfg MODEL_DEFAULT)" ]; then
...
cmd_show() {
  ...
  echo "  Provider: $tier"
  echo "  Mode:     $mode"
  local dflt; dflt="$(sprintmd_cfg MODEL_DEFAULT)"
...
# ── list ─────────────────────────────────────────────────────────────
cmd_list() {
  local tier; tier="$(sprintmd_ai_tier)"
  echo -e "${BOLD}Models available on this provider${NC} ($tier)"
  echo ""
  case "$tier" in
    grok-build)
      if command -v "$SPRINTMD_CLI" >/dev/null 2>&1; then
        # Grok Build exposes a live model list; show it verbatim.
        if "$SPRINTMD_CLI" models 2>/dev/null; then
          echo ""
          echo -e "${DIM}Pin one with:  ./sprint.sh model set default <model>${NC}"
          return 0
        fi
        echo -e "${YELLOW}\`$SPRINTMD_CLI models\` returned nothing — showing known aliases.${NC}"
        echo ""
      fi
      ...
      echo ""
      echo "Run \`$SPRINTMD_CLI models\` for the authoritative, up-to-date list."
      ;;
    claude-code)
    ...
    *)
      ...
      echo "Use a model id your CLI ($SPRINTMD_CLI) understands; see its own docs."
      ;;
  esac
  echo ""
  echo -e "${DIM}Pin one with:  ./sprint.sh model set default <model>${NC}"
...
# MODEL_DEFAULT / MODEL_<ROLE> via sprintmd_cfg_set so no unrelated key is
...
cmd_set() {
  ...
  sprintmd_cfg_set "$cfg_key" "$value"
  if [ -n "$value" ]; then
    echo -e "${GREEN}✓${NC} Set $cfg_key=$value in docs/sprintmd/config"
  else
    echo -e "${GREEN}✓${NC} Cleared $cfg_key in docs/sprintmd/config (falls back to the next layer)"
  fi
