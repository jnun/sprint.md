...
# the whole audit. Override with SPRINTMD_DEPS_TIMEOUT.
DEPS_TIMEOUT="${SPRINTMD_DEPS_TIMEOUT:-120}"

# ── Preflight ───────────────────────────────────────────────────────
if [ ! -f "docs/sprintmd/DOC_STATE.md" ]; then
  echo -e "${RED}ERROR: docs/sprintmd/DOC_STATE.md not found!${NC}" >&2
...
# stable, grouped output. Override the vendor list with SPRINTMD_DEPS_PRUNE.
PRUNE_DIRS="${SPRINTMD_DEPS_PRUNE:-node_modules vendor .git dist build out target .venv venv env .tox .next .nuxt .svelte-kit .cache coverage bower_components Pods .terraform __pycache__ .gradle}"
...
MAX_PROJECTS="${SPRINTMD_DEPS_MAX_PROJECTS:-25}"
PROCESSED=""
COUNT=0
SKIPPED=0
...
if [ "$SKIPPED" -gt 0 ]; then
  ...
  echo "  Raise it with SPRINTMD_DEPS_MAX_PROJECTS=<n> to cover them."
fi
...
# not sprintmd_emitted — the exec path runs sprintmd_run in a command
...
_budget_args=()
[ -n "${SPRINTMD_BUDGET_DEPS:-}" ] && _budget_args=(--budget "$SPRINTMD_BUDGET_DEPS")

LOG_FILE="$(sprintmd_log_path deps "$(basename "$TASK_FILE")")"
...
VERDICT=$(printf '%s' "$OUTPUT" | sprintmd_parse_verdict 'FILED|CLEAN')
