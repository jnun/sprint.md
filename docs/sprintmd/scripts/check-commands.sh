...
#   1. the registry   docs/sprintmd/help/_registry   (the source of truth)
#   2. the dispatch   sprint.sh  (the case arms that route to cmd_* functions)
#   3. the help pages docs/sprintmd/help/<cmd>.md
...
if [ -n "$_missing" ]; then
    ...
    for c in $_missing; do echo "    $c   — create docs/sprintmd/help/$c.md"; done
    echo ""
fi
