...
# Test 5: consumers use sprintbias_profile_line (no inlined pointer left)
echo "Test 5: create-idea / create-feature use shared helper"
idea="$SPRINTBIAS_SRC/scripts/create-idea.sh"
feat="$SPRINTBIAS_SRC/scripts/create-feature.sh"
if grep -q 'sprintbias_profile_line' "$idea" && ! grep -q 'Also read docs/sprintbias/project.md' "$idea"; then
...
if grep -q 'sprintbias_profile_line' "$feat" && ! grep -q 'Also read docs/sprintbias/project.md' "$feat"; then
