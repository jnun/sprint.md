...
PASS=0
FAIL=0
SCRIPT_UNDER_TEST="$(cd "$(dirname "$0")/../sprintmd/scripts" && pwd)/create-bug.sh"
...
setup() {
    ...
    cp "$SCRIPT_UNDER_TEST" "$TMPDIR/docs/sprintmd/scripts/create-bug.sh"
...
setup
(cd "$TMPDIR" && bash docs/sprintmd/scripts/create-bug.sh "Login button broken" > /dev/null 2>&1)
...
setup
if (cd "$TMPDIR" && bash docs/sprintmd/scripts/create-bug.sh "" > /dev/null 2>&1); then
...
if (cd "$TMPDIR" && bash docs/sprintmd/scripts/create-bug.sh "Some bug" > /dev/null 2>&1); then
    echo "  FAIL: Should have exited non-zero"
    FAIL=$((FAIL + 1))
else
...
setup
(cd "$TMPDIR" && bash docs/sprintmd/scripts/create-bug.sh "Date check" > /dev/null 2>&1)
