...
PASS=0
FAIL=0
SCRIPT_UNDER_TEST="$(cd "$(dirname "$0")/../sprintmd/scripts" && pwd)/ai-context.sh"
...
setup() {
    ...
    cp "$SCRIPT_UNDER_TEST" "$TMPDIR/docs/sprintmd/scripts/ai-context.sh"
...
setup
output=$(bash "$TMPDIR/docs/sprintmd/scripts/ai-context.sh" 2>&1)
